<?php
declare(strict_types=1);
namespace Quid\Main\Test;
use Quid\Main;
use Quid\Base;

// std
class Std extends Base\Test
{
	// trigger
	public static function trigger(array $data):bool
	{
		// construct
		$arr = new Main\Std();
		
		// map
		assert($arr->push(array('okv'=>1),1) === $arr);
		assert($arr->are(0,1));
		assert($arr->unshift(array('okv'=>1),1) === $arr);
		assert($arr->append(array('okz'=>1),1) === $arr);
		assert($arr->prepend(array('okz'=>3),9) === $arr);
		assert($arr->isCount(7));
		assert($arr->pop(2) === array(1,1));
		assert($arr->shift() === 1);
		assert($arr->isCount(4));
		assert($arr->replace(array(3=>array('r'=>true))) === $arr);
		assert($arr[3]['r'] === true);
		assert($arr->splice(0,1)->isCount(2));
		assert($arr->spliceIndex(0,1)->isCount(1));
		assert($arr->insert(0,array('ok'=>2)) === $arr);
		assert($arr->insertIndex(0,array('ok'=>3)) === $arr);
		assert($arr->isCount(2));
		assert($arr['ok'] === 3);
		$new = $arr->filter(function($value,$key) {
			return (is_int($value))? true:false;
		});
		assert($new->count() === 1);
		assert($new->map(function($value) {
			return (is_int($value))? true:$value;
		})['ok'] === true);
		$base = new Main\Std(array(1,'james','ok'=>'test'));
		$base2 = new Main\Std(array(1,2,3));
		assert(!$base->isIndexed());
		assert($base2->isIndexed());
		assert(!$base->isSequential());
		assert($base2->isSequential());
		assert($base->isAssoc());
		assert(!$base2->isAssoc());
		assert($base->hasNumericKey());
		assert($base->hasNonNumericKey());
		assert(!$base->hasKeyCaseConflict());
		assert($base->isUni());
		assert(!$base->isMulti());
		assert(!$base->onlyNumeric());
		assert($base2->onlyNumeric());
		assert(!$base->isSet());
		assert($base2->isSet());
		$count = new Main\Std(array(1,2,3,4,5));
		assert($count->unsetAfterCount(4)->count() === 4);
		$sort = new Main\Std(array('test'=>'ok','james'=>'deux'));
		assert($sort->sort(true)->first() === 'deux');
		assert($sort->sort(false)->first() === 'ok');
		$sequential = new Main\Std(array('test'=>'ok','james'=>'deux'));
		assert(!$sequential->isSequential());
		assert($sequential->sequential()->isSequential());
		
		return true;
	}
}
?>