...
class Lang extends Base\Test
{
	...
	public static function trigger(array $data):bool
	{
		...
		// construct
		$lang = new Main\Lang(array('fr','en','de'),array('onLoad'=>function(string $value) { }));
		assert($lang->overwrite($fr::$config)->isNotEmpty());
		$lang['test'] = array('ok'=>array('test'=>'what'));
		...
		// othersLang
		assert($lang->othersLang() === array('fr','de'));
		assert($lang->othersLang('fr') === array('en','de'));

		// allLang
		assert($lang->allLang() === array('fr','en','de'));
		...
		// setLang
		assert($lang->setLang('fr',array('fr','en','de','ge')) instanceof Main\Lang);
		...
		$lang->replace(array('lol'=>array('lol2'=>false,'lol1'=>1,'lol3'=>'oui','lol4'=>'non','lol5'=>array(1,'ok'=>'okfr'),'fr'=>'yepfr','replace'=>'Mon%l% [remplacement]')));
		$lang->changeLang('en')->replace(array('lol'=>array('lol2'=>true,'lol1'=>2,'lol3'=>'yes','lol4'=>'no','lol5'=>array(1,'ok'=>'oken'),'en'=>'yepen','replace'=>'My%l% [replace]')))->changeLang('fr');
		...
		// takes
		assert($lang->takes(array(array('lol/lol2'),'lol/lol1')) === array('lol/lol2'=>false,'lol/lol1'=>1));
		assert($lang->takes(array(array('lol/lol2'),'lol/lol1'),'en') === array('lol/lol2'=>true,'lol/lol1'=>2));

		// takeUnpack
		assert($lang->takeUnpack(array('lol/lol1','fr')) === 1);
		assert($lang->takeUnpack(array('lol/lol1','en')) === 2);

		// getAppend
		assert($lang->getAppend('lol','lol1') === 1);

		// getAll
		assert($lang->getAll('lol/lol1') === array('fr'=>1,'en'=>2));

		// getOthers
		assert($lang->getOthers('lol/lol1') === array('en'=>2));

		// text
		assert($lang->text('lol/lol1') === '1');
		assert($lang->text('lol/lol1',null,'en') === '2');
		assert($lang->text('lol/replace',array('remplacement'=>'OK')) === 'Mon%l% OK');
		assert($lang->text('lc|lol/replace') === 'mon%l% [remplacement]');
		assert($lang->text('lc|lol/replace',null,null,array('pattern'=>3)) === 'mon');
		assert($lang->text('lc|lol/replace',null,null,array('pattern'=>4)) === 'm...');
		assert($lang->text(array('lol','replace'),null,null,array('case'=>'lc')) === "mon%l% [remplacement]");

		// textAfter
		assert($lang->textAfter('ok',array('pattern'=>'%lol','html'=>'div')) === '<div>ok</div>lol');
		assert($lang->textAfter('ok') === 'ok');

		// textOption
		assert(count($lang->option()) === 4);
		assert(count($lang->textOption(array('error'=>false))) === 9);
		assert($lang->textOption(array('error'=>false))['error'] === false);
		...
		assert($lang->textAppendOne(array('lol'),'lol5/ok') === 'okfr');
		assert($lang->textAppendOne('tc|lol','lol3') === 'Oui');

		// textAppend
		assert($lang->textAppend('lol','lol5','ok') === 'okfr');
		assert($lang->textAppend('lol',array('lol5','ok')) === 'okfr');

		// textUnpack
		assert($lang->textUnpack(array('lol/lol5/ok',null,'en')) === 'oken');
		assert($lang->textUnpack(array('lol/replace',array('remplacement'=>'OK'))) === 'Mon%l% OK');
		assert($lang->textUnpack(array('lol/replace',array('replace'=>'OK'),'en')) === 'My%l% OK');

		// textAll
		assert($lang->textAll('lol/replace') === array('fr'=>'Mon%l% [remplacement]','en'=>'My%l% [replace]'));
		assert($lang->textAll('lol/replace',array('remplacement'=>'OK','replace'=>'OK')) === array('fr'=>'Mon%l% OK','en'=>'My%l% OK'));

		// textOthers
		assert($lang->textOthers('lol/replace') === array('en'=>'My%l% [replace]'));
		assert($lang->textOthers('lol/replace',array('replace'=>'OK')) === array('en'=>'My%l% OK'));

		// plural
		assert($lang->plural(2,'lol/replace') === 'Mon%l% [remplacement]s');
		assert($lang->plural(1,'lol/replace') === 'Mon%l% [remplacement]');
		assert($lang->plural(array(2,3,4),'lol/replace',null,array('l'=>'w')) === 'Monw [remplacement]');
		assert($lang->plural(1,'lol/replace',null,array('l'=>'YYY')) === 'Monl [remplacement]');
		assert($lang->plural(2,'lol/replace',null,array('l'=>'YYY'),'en') === 'MyYYY [replace]');
		assert($lang->plural(1,'lol/replace',null,array('l'=>'YYY'),'en') === 'Myl [replace]');
		assert($lang->plural(2,'lcf|lol/replace',null,array('l'=>'YYY'),'en') === 'myYYY [replace]');
		assert($lang->plural(2,'lc|lol/replace',null,array('l'=>'YYY'),'en') === 'myYYY [replace]'); // le remplacement vient avant plural
		assert($lang->plural(2,'plural/1') === 'tests oks');
		assert($lang->plural(2,'plural/2') === 'testas');

		// html
		assert($lang->html('div','lol/lol3') === '<div>oui</div>');
		assert($lang->html(array('div',array('#myId','element')),'lol/lol3') === "<div id='myId' class='element'>oui</div>");
		assert($lang->html(array('div',array('#myId','element')),'lol/lol3',null,'en') === "<div id='myId' class='element'>yes</div>");
		assert($lang->html(array('div',array('#myId','element')),'ucf|lol/lol3',null,'en') === "<div id='myId' class='element'>Yes</div>");
		...
		// translate
		assert($lang->translate('non') === array('lol/lol4'=>'no'));
		assert($lang->translate('no','fr','en') === array('lol/lol4'=>'non'));
		...
		assert($lang->sets(array('lol/lol2/ok2'=>'value2','lol/lol2/ok3'=>'value3')) instanceof Main\Lang);
		$lang['test'] = 2;
		assert($lang['test'] === 2);
		unset($lang['test']);
		...
		// map
		assert($lang->index(array(-1,-1,-1)) === 'value3');
		assert($lang->set('lol/lol2/ok','value') instanceof Main\Lang);
		assert($lang->sets(array('lol/lol2/ok2'=>'value2','lol/lol2/ok3'=>'value3')) instanceof Main\Lang);
		assert($lang->exists('lol/lol2/ok','lol/lol2/ok2'));
		assert(!$lang->exists('lol/lol2/ok','lol/lol2/ok4'));
		assert($lang->gets('lol/lol2/ok',array('lol','lol2','ok2')) === array('lol/lol2/ok'=>'value','lol/lol2/ok2'=>'value2'));
		assert(Base\Arr::isMulti($lang->values()));
		assert($lang->in('value3'));
		assert($lang->inFirst('value4','value3') === 'value3');
		assert($lang->existsFirst('lol2',array('lol','lol2','ok')) === array('lol','lol2','ok'));
		assert(count($lang->slice('lol',true)) === 1);
		assert($lang->replace(array('lol'=>array('final'=>true)))['lol']['final'] === true);
		...
		assert($lang->set(array('lol',null,2,null),'new')->get('lol/0/2/0') === 'new');
